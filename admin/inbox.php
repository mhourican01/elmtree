<?php
	...
	// selects messages sent to user
	if(isset($_GET['myinbox'])){
		$myinbox = $_GET['myinbox'];
		
		$readinbox = "SELECT * FROM 1ET_Messages
		INNER JOIN 1ET_Users
		ON 1ET_Messages.Sender=1ET_Users.ID 
		WHERE Receiver='$myinbox'
		ORDER BY Date DESC;
		";
		
		$inboxresult = $conn->query($readinbox);
...
<html>
  <head>
	...
	<section>
		<p class='title'>My received messages</p>
		<?php
		while($inboxrow=$inboxresult->fetch_assoc()) {
			$sender = $inboxrow['Sender'];
			$msg = $inboxrow['Message'];
			$date = $inboxrow['Date'];
			
			$readem = "SELECT EmailAddress FROM 1ET_Users 
			INNER JOIN 1ET_Messages 
			ON 1ET_Users.ID=1ET_Messages.Sender 
			WHERE Sender=$sender
			";
	
			$readresult = $conn->query($readem);
	
			if (!$readresult) {
				$conn->error;
			}
			
			while($emrow = $readresult->fetch_assoc()) {
				$emsend = $emrow['EmailAddress'];
			}
				echo "
				<div class='column msgctnr'>
					<p class='subtitle'>From <strong>$emsend</strong> on <strong>$date</strong>:</p>
					<p class='subtitle'>$msg</p>
					<a href='profile.php?sender=$sender' class='button is-primary'>View $emsend's profile</a>
				</div>
			";		
		}
		?>
